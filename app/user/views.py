...
def send_account_email(user, subject, message):
    """Send through Django's configured backend and report delivery failure."""
    try:
        return bool(
            send_mail(
                subject,
        ...
        logger.exception("Unable to send account email to %s", user.email)
        return False
...
def send_verification_email(request, user):
    return send_account_email(
        ...
        f"{build_verification_url(request, user)}",
    )
...
class RegisterView(APIView):
    ...
    def post(self, request):
        ...
        verification_email_sent = send_verification_email(request, user)
        response_data = build_token_response(user)
        response_data["verification_email_sent"] = verification_email_sent
...
class ResendVerificationView(APIView):
    def post(self, request):
        ...
        # Keep this response generic so emails cannot be used to enumerate
        # accounts in the system.
        return Response(
            {"detail": "If the account needs verification, an email has been sent."}
        )
...
class ForgotPasswordView(APIView):
    ...
    def post(self, request):
        ...
        if user:
            ...
            token = default_token_generator.make_token(user)
            reset_url = (
                request.build_absolute_uri("/api/user/reset-password/")
...
class GoogleCallbackView(APIView):
    ...
    def get(self, request):
        ...
        if not expected_state or not received_state:
            return Response(
                {"detail": "Invalid Google login session."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        # Google has confirmed ownership of this address, including for an
        # existing account originally created with email/password.
        if not user.is_verified:
            user.is_verified = True
            user.save(update_fields=["is_verified"])
